...
class UsuarioDao:
    ...
    def salvar(self, id):
        cursor = self.__db.connection.cursor()
        cursor.execute(SQL_USUARIO_POR_ID, (id.id,))
        dados = cursor.fetchone()
        print(dados)
        verify = True if dados else False
        print(verify)
        if verify:          #Se verificar um usuário existente, é retornado um false para a tentativa de criação de usuário
            return False
        else:
            cursor.execute(SQL_CRIA_USUARIO, (id.id, id.nome, id.senha))
            self.__db.connection.commit()
            return True
